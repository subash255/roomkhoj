...
<div class="container mx-auto p-6">
            ...
            @foreach($selectpayments as $payment)
                @if($payment->room_id == $room->id) <!-- Ensure this is the current room -->
                    <div class="flex items-center mt-4">
                        <img src="{{ asset('image/rooms/' . $payment->photopath) }}" alt="Room Image" class="w-24 h-24 object-cover rounded-lg mr-4">
                        <div>
                            ...
                            <p class="text-lg">Booked by: {{ auth()->user()->name }}</p>
    ...
    <form action="{{ route('users.stores', $room->id) }}" method="POST"> <!-- Pass room id to the route -->
        @csrf
